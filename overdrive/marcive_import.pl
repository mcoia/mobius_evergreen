...
sub importMARCintoEvergreen
{
	...
	while ( my $marc = $file->next() ) 
	{
		...
		if(1)#$r>8686)#$overlay<16)
		{
			...
			{
				@ret = @{chooseWinnerAndDeleteRest($bibid, $dbHandler, $marc, \@notworked, \@updated, $log)};
				@updated = @{@ret[0]};
				@notworked = @{@ret[1]};
				$overlay+=$#updated+1;
			}
...
sub chooseWinnerAndDeleteRest
{
    ...
    my $winnerBibID;
    my $ogmarcxml;
    $title = $finalMARC->field('245')->subfield('a');
	...
	recordBIBMARCChanges($winnerBibID, $ogmarcxml, $finalMARC);
	my @values = ($finalMARC);
	my $query = "UPDATE BIBLIO.RECORD_ENTRY SET marc=\$1 WHERE ID=$winnerBibID";
...
{
	my $marcsearch = @_[0];
	my $bibid = $marcsearch->field('901')->subfield('a');
	my $dbHandler = @_[1];
	my $log = @_[2];

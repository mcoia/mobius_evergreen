...
When you are ready to transmit your bib record file for Overnight Authorities processing, log into our FTP server at ftp://ftp.marcive.com/  as “anonymous” and using your e-mail address as the password. 
  The data should be placed in the “input\athority\” subdirectory creating a file named “MIEVmmdd.BIB” 
 where "MIEV" is your Marcive ID code,  “mm” is the current month (e.g., “06” for June) and “dd” is the day of the month.  For example if your file was being sent today, the file name would be "MIEV0223.BIB”. 
 An email message should be sent to production@marcive.com that includes the file name and record count once you have FTP’d your file.
Upon completion of the processing of the file sent for Overnight Authorities processing, we will return the processed bib records, new authority records and reports. 
 An email will be supplied that provides the location of these files. 
 All files are available for a period of 14 days.  The email will list the date the files will be removed from the server.

The files generated by MARCIVE  for  Overnight Authorities will follow the following naming convention. The “XXX” represents a file number.  The  “RELATED” files are the reports.
...
RELATED4.XXX
All processed files will be output to the following directory ftp://ftp.marcive.com/output/ftp/XXXXXX This is the output directory that was used for the backfile project.

For the CNS bib record history you can make deletions by supplying a list of the control numbers in the 001 of the bib record.  Please name this file MIEV_CNS_BIB.del. 
 The cost for this processing is $.05 per bib/$10 min/$50 max.  This will need to be done as soon as possible before the update. 
 This file should also be placed in the “input\athority\” subdirectory and an email should be sent to production@marcive.com to include the record count and file name.

If you have made changes to the bib records, it is necessary for you to supply MARCIVE with any updated records you have so that we do not supply you with an older copy of those records. 
If you have a very large number of changed records or you do not have a way to identify the records that you have modified,
 you could resend your full catalog. There is an extra fee for this method.


Generally, all you will need to do is send us an update file through Overnight Authorities a couple of weeks before we run your Comprehensive Notification.  You will receive email alerts  before each Comprehensive Notification run. 
 This Comprehensive Notification run is scheduled for processing the first week of March.
 Please let us know by next week if  you will be sending an update file to be included with that CN processing so that we can best advise you on the cut-off date.

 

The file names for the Comprehensive Notification data will be as follows.  Again “XXX” is a file number.  No reports are generated as part of the CNS service.    

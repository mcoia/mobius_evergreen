...
use Getopt::Long;
	...
	my $file = @ARGV[0];
    my $tablenameforschema = @ARGV[1];
	$log = new Loghandler("/mnt/evergreen/migration/festus/log/extract.log");
	$log->truncFile("");
	...
	while ((my $table, my $search ) = each(%loopvars))
	{
		...
		my $makeTable = "CREATE TABLE m_festpl.$table (";
		my @billfinal = ();
		my @billsections = ();

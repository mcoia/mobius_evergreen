...
in(select id from asset.copy where barcode in(select barcode from m_scenic.asset_copy_legacy where l_status='Lost & Paid')))
...
";
		...
		$log->addLogLine("Updated $updatecount / $total payments");

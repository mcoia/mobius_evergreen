    ...
    if ($conf{"logfile"})
    {
        ...
        if($valid)
        {
            ...
            if($records)
            {
                ...
                {
                    my $cmd = '"curl --form \\"file=@' . $outputFile->getFileName() . '\\" ' . $conf{"httpuploadurl"}. '"';
                    $cmd = "system($cmd);";
                    ...
                    if ($@)
                    {
                        ...
                        my $email = new email($conf{"fromemail"},\@tolist,1,0,\%conf);
                        $email->send("RMO marcive deleted bib HTTPS FAIL $fdate","I'm just going to apologize right now, I could not FTP the file to ".$conf{"httpuploadurl"}." ! Remote directory: $remoteDirectory\r\n\r\nYou are going to have to do it by hand. Bummer.\r\n\r\nCheck the log located: ".$conf{"logfile"}." and you will know more about why. Please fix this so that I can FTP the file in the future!\r\n\r\n -MOBIUS Perl Squad-");
                        $valid=0;
                    }
                }
                if($valid)
